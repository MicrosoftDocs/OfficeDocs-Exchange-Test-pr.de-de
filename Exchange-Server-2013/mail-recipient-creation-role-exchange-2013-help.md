---
title: 'Rolle „Erstellung von E-Mail-Empfängern“: Exchange 2013 Help'
TOCTitle: Rolle „Erstellung von E-Mail-Empfängern“
ms:assetid: 81081845-9ebf-4888-8f9f-fe7cf2704486
ms:mtpsurl: https://technet.microsoft.com/de-de/library/Dd876915(v=EXCHG.150)
ms:contentKeyID: 50476123
ms.date: 04/24/2018
...
Mithilfe dieser Rolle können keine E-Mail-Aktivierung von Öffentlichen Ordnern durchführen. Zur E-Mail-Aktivierung Öffentlicher Ordner muss die Rolle `Mail Enabled Public Folders` verwendet werden. Weitere Informationen finden Sie unter [Rolle „E-Mail-fähige öffentliche Ordner“](mail-enabled-public-folders-role-exchange-2013-help.md).

Wenn Ihre Organisation mit einem Berechtigungsmodell für die rollenbasierte Zugriffssteuerung (Role Based Access, RBAC) arbeitet, bei dem die Empfängererstellung von einer anderen Gruppe durchgeführt wird als die Empfängerverwaltung, weisen Sie die Rolle `Mail Recipient Creation` der Verwaltungsrollengruppe zu, die für die Empfängererstellung zuständig ist. Die Rolle `Mail Recipients` sollte der Rollengruppe zugewiesen werden, die für die Empfängerverwaltung verantwortlich ist.

Wenn Ihre Organisation das geteilte Active Directory-Berechtigungsmodell aktiviert hat, wurden alle nicht delegierenden Verwaltungsrollenzuweisungen für diese Verwaltungsrolle entfernt. Wenn geteilte Active Directory-Berechtigungen aktiviert sind, können nur Active Directory-Administratoren mithilfe von Active Directory-Verwaltungstools neue Sicherheitsprinzipale wie Benutzer und Sicherheitsgruppen erstellen.

Weitere Informationen zur rollenbasierten Zugriffssteuerung und zu geteilten Active Directory-Berechtigungen finden Sie unter [Grundlegendes zu geteilten Berechtigungen](understanding-split-permissions-exchange-2013-help.md).
...
## Zusätzliche Überlegungen zum Gültigkeitsbereich

Für das in dieser Rolle enthaltene Cmdlet **New-Mailbox** werden nicht nur Empfängerbereiche, sondern auch Datenbankkonfigurationsbereiche festgelegt. Datenbankkonfigurationsbereiche steuern, in welchen Datenbanken über das Cmdlet neue Postfächer erstellt werden können. Die Datenbank, in der ein Postfach erstellt werden soll, muss sich innerhalb des Datenbankbereichs befinden. Diese Anforderung gilt, wenn Sie eine Datenbank mithilfe des Parameters *Database* im Cmdlet **New-Mailbox** angeben oder die Datenbankauswahl durch eine automatische Postfachverteilung zulassen. Weitere Informationen finden Sie unter [Grundlegendes zu Verwaltungsrollenbereichen](understanding-management-role-scopes-exchange-2013-help.md).
